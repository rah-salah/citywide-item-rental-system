...
(function () {
  ...
  // Expose i18n + theme helpers for ui-enhancements.js (dashboard controls)
  try {
    window.cwSetLang = setLang;
    window.cwGetLang = getLang;
    window.cwApplyLang = applyLang;
  } catch(e){}
  ...
  function enhanceNavbar() {
    ...
    if (u) {
      ...
      if (!hardcodedDrop && !btnRow.querySelector(".cw-profile-drop")) {
        ...
        drop.innerHTML =
          '<button class="cw-avatar cw-avatar-sm cw-profile-btn dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false" title="'+ (u.name||'') +'">'+
            initials(u.name) +
          '</button>' +
          '<ul class="dropdown-menu dropdown-menu-end cw-profile-menu">' +
            '<li class="px-3 py-2">' +
              '<div class="d-flex align-items-center">' +
                '<span class="cw-avatar cw-avatar-sm me-2">'+initials(u.name)+'</span>' +
                '<div class="small"><div class="fw-semibold">'+ (u.name||'') +'</div>' +
                '<div class="text-muted">'+ (u.email||'') +'</div></div>' +
              '</div>' +
            '</li>' +
...
(function(){
  ...
  function renderSidebar(host, groups){
    ...
    var logoSrc = inSection ? "../logo-header.svg" : "logo-header.svg";
    var html = '<div class="cw-sidebar-logo"><img src="'+logoSrc+'" alt="Citywide"/></div>'+
               ...
               '<div class="text-muted small">'+area+'</div></div></div>';
...
(function(){
  ...
  function renderUserSidebars(){
    ...
    document.querySelectorAll("[data-cw-sidebar]").forEach(function(host){
      ...
      html += '<div class="px-2 mb-3 d-flex align-items-center gap-2">'+
              avatarHTML(u)+
              '<div><div class="fw-semibold small">'+(u.name||"User")+'</div>'+
              '<div class="text-muted small">'+(u.area||"Jigjiga Central")+'</div></div></div>';
      var groups = [];
  ...
  function wireProfileName(){
    ...
    f.addEventListener("submit", function(e){
      ...
      if(!n) return;
      var u = sget(); if(!u) return;
      u.name = n.trim(); sset(u); renderUserSidebars();
...
(function(){
  ...
  function render(host, groups){
    ...
    var html = '<div class="cw-sidebar-logo"><img src="../logo-header.svg" alt="Citywide"/></div>'+
               '<div class="px-2 mb-3 d-flex align-items-center gap-2">'+avatar+
...
(function(){
  ...
  var DEFAULT_AD_SLIDES = [
    { id:"ad-1", type:"ad", active:true, image:"images/cameras.jpg",
      title:"Professional Camera Kit",
      description:"Capture every moment with premium photography equipment.",
      ctaText:"View Listing", ctaHref:"view-details.html" },
    { id:"ad-2", type:"ad", active:true, image:"images/generators.jpg",
      title:"Honda 5kW Generator",
      description:"Reliable backup power for events, shops and construction sites.",
      ctaText:"View Listing", ctaHref:"view-details.html" },
    { id:"ad-3", type:"ad", active:true, image:"images/speaker.jpg",
  ...
  var VALUE_SLIDES = [
    { id:"val-1", type:"value", image:"images/event-gear.jpg",
    ...
    { id:"val-2", type:"value", image:"images/power-tools.jpg",
  ...
  function renderSuperAnalytics(){
    ...
    loadScript("https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js").then(function(){
      ...
      var palette = ["#0d6efd","#198754","#ffc107","#dc3545","#6f42c1","#20c997","#fd7e14","#0dcaf0"];
